...
## 关于optimization
这里主要包含四个类分别是EFResidual、EFFrame、EFPoint以及EFFunctional。这个四个类的主要EFResidual主要负责计算残差，成员函数很简单，仅有takeDataF()。该函数用来计算JI_JI_Jd，EFFrame、EFPoint都有这个函数。

另外三个类主要提供了成员变量，这些数据的操作在EFFunctional中，四个类的成员变量如表所示
![](/img/dso-slam-varaince)
他们之间的关系如下图所示
